...
async def info(event):
    await event.edit("**🔁 Analizzando la chat...**")
    ...
    try:
        ...
        print("**❌Errore:**", e)
        await event.edit("`Si è verificato un errore imprevisto.`")
    return
...
async def get_chatinfo(event):
    ...
    try:
        ...
        try:
            ...
            await event.edit("**❌Errore:** `In questo in gruppo/canale sono bannato (oppure è privato).`")
            return None
...
async def fetch_info(chat, event):
    ...
    if hasattr(chat_obj_info, "restretto"):
        ...
        if chat_obj_info.restricted:
            ...
            caption += f"> Testo: {chat_obj_info.restriction_reason[0].text}\n\n"
        else:
            caption += "\n"
...
CMD_HELP.update({
        "chatinfo":
        ".chatinfo [optional: <reply/tag/chat id/link>]\
            \nAlcune informazioni potrebbero essere limitate a causa di autorizzazioni mancanti."
})
