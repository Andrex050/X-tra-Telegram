...
if Var.PRIVATE_GROUP_ID is not None:
    ...
    async def approve_p_m(event):
        ...
        if event.is_private:
            if not pmpermit_sql.is_approved(chat.id):
                ...
                await event.edit("💭 [{}](tg://user?id={}) **approvato a scrivermi in PM.*".format(firstname, chat.id))
        ...
        if event.is_private:
            if not pmpermit_sql.is_approved(chat.id):
                if not chat.id in PM_WARNS:
                    ...
                    bruh = "**💭 Utente automaticamente approvato.**"
    ...
    async def approve_p_m(event):
        ...
        APPROVED_PMs = "**💭 Lista utenti approvati:**\n"
    ...
    async def do_pm_permit_action(chat_id, event):
        ...
        if PM_WARNS[chat_id] == Config.MAX_FLOOD_IN_P_M_s:
            ...
            the_message += f"Messaggi inviati: {PM_WARNS[chat_id]}\n"
