...
async def _(event):
    ...
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if reason:
            await borg.send_message(event.chat_id, f"**⚠️ Modalità AFK attivata!**\n💭 **Motivo:** `{reason}.`")
        else:
            await borg.send_message(event.chat_id, f"**⚠️ Modalità AFK attivata!**")
...
async def set_not_afk(event):
    ...
    if ".afk" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        shite = await borg.send_message(event.chat_id, "**💭 Sono tornato!**\n**⚠️ Modalità AFK disattivata.**\n**ℹ️ Sono stato AFK per:** `" + total_afk_time + " `")
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        if afk_time:  # pylint:disable=E0602
            ...
            if days == 1:
                ...
                afk_since = f"`{int(hours)}h{int(minutes)}` **fa**"
            elif minutes > 0:
                afk_since = f"`{int(minutes)}m{int(seconds)}` **fa**"
            else:
                afk_since = f"`{int(seconds)}` **fa**"
        msg = None
        message_to_reply = f"**⚠️ Andrex è AFK!**\nℹ️ **AFK da:** `{total_afk_time}.`" + \
            f"\n\n📚 **Motivo:** `{reason}`." \
            if reason \
